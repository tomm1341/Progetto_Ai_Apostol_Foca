...
class NetRunner():
    ...
    def __init__(self, cfg_object: object, classes: list[str]) -> None:
        ...
        if self.cfg.train_parameters.reload_last_model:
            try:
                self.net.load_state_dict(torch.load(self.best_model_outpath_sd, weights_only=True))
    ...
    def train(self) -> None:
        ...
        for epoch in range(epochs):
            ...
            for i, data in enumerate(self.tr_loader, 0):
                ...
                # L'input attraversa la rete. Errori vengono commessi.
            ...
            if early_stop_check and (epoch + 1) % es_loss_evaluation_epochs == 0:
                ...
                # - Se non lo e', aggiorno il counter dei NON MIGLIORAMENTI.
    ...
    def test(self, loader: torch.utils.data.DataLoader = None, use_current_net: bool = False, preview : bool = False, print_acc: bool = False):
        ...
        if use_current_net:
            ...
            try:
                net.load_state_dict(torch.load(self.best_model_outpath_sd, weights_only=True))
            except:
    ...
    def __get_net(self):
        ...
        if self.cfg.train_parameters.network_type.lower() == 'net_2':
    ...
    def __load_data(self) -> None:
        ...
        tr_dataset = CustomDatasetVehicles(root=self.cfg.io.training_folder, transform=transforms)
        cp.cyan(f'Analyzing validation dataset: {self.cfg.io.validation_folder}')
        va_dataset = CustomDatasetVehicles(root=self.cfg.io.validation_folder, transform=transforms)
        cp.cyan(f'Analyzing test dataset: {self.cfg.io.test_folder}')
        te_dataset = CustomDatasetVehicles(root=self.cfg.io.test_folder, transform=transforms)
